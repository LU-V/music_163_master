...
def save_artist(group_id, initial):
    ...
    # 网页解析
    soup = BeautifulSoup(r.content.decode(), 'html.parser')##使用BeautifulSoup解析这段代码 解析格式是'html.parser'
    body = soup.body

    hot_artists = body.find_all('a', attrs={'class': 'msk'})#find_all() 方法搜索当前tag的所有tag子节点,并判断是否符合过滤器的条件
    artists = body.find_all('a', attrs={'class': 'nm nm-icn f-thide s-fc0'})#获取a标签的class属性
